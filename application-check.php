...
if(isset($_POST['check'])){
   $email=$_POST['email'];
    ...
    $qu=mysqli_fetch_array($ta);
        ...
        if($qu){
           if ($qu['status'] === '1') {
           	echo "<script>alert('You have been nominated for admission, go to our portal and register (https://www.nacest.edu/register)')</script>";
           } else {
           	echo "<script>alert('You have not been nominated offered admission yet..check another time!!!')</script>";
           }
